...
const SelectedCourses = ({selectedCourses, creditHour}) => {
  const totalCreditHours = selectedCourses.reduce((total, curr)=> total + curr.credit, 0)
  const totalPrice = selectedCourses.reduce((total, curr)=>total + curr.price, 0)
  return (
    <div className='p-5 bg-[#f5f5f5] space-y-3'>
      ...
      <hr />
      <h3 className='font-bold'>Total Price : {totalPrice} USD</h3>
    </div>
  )
}
